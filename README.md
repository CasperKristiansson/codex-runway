...
- Adds an Analytics tab with usage history, top chats, plan-limit periods, plugin and skill activity, and messages by model or surface. Choose one saved account or combine enabled accounts, and switch charts between 7 days, 30 days, and one year. The combined usage percentage is weighted by each account's plan capacity; message and tool counts are summed. Missing accounts are identified instead of being treated as zero usage.
- Collects the signed-in account's Analytics alongside the 15-minute dashboard refresh. A first sync and weekly backfill request one year in bounded date windows; intervening syncs re-read 30 days so revised daily values replace prior rows. Top chats are checked at most every six hours using local task metadata. Per-account Analytics is kept for one year in `~/Library/Application Support/Codex Runway/Analytics`; the archive is written atomically and contains no access token. Other accounts keep their last saved Analytics until signed in again.
- In Settings, choose a backup folder to start automatic daily ZIP snapshots. Runway checks on launch, every 30 minutes while running, and after wake; it creates at most one scheduled backup per local calendar day. "Back Up Now" can create another copy at any time. Daily copies older than the selected retention period (30 days by default) are removed after a successful backup. The first backup of each month is also saved as a permanent monthly copy; Runway never deletes monthly copies. The chosen folder contains a `Codex Runway Backups` subfolder.
...
Saved data stays local unless you choose a folder synced by another service. Backups are ordinary, unencrypted ZIP files containing Runway account settings and saved usage history; they do not include Codex authentication files or access tokens. If you select OneDrive, its client controls whether and when the backup reaches the cloud. Codex Runway uses the local Codex App Server's read-only account, rate-limit and profile-usage methods, which fetch data from OpenAI. For Analytics, it reads the current Codex access token from `~/.codex/auth.json` at sync time and calls the same first-party Analytics routes as the installed Codex app. It does not store the token, read browser cookies, switch accounts, or send data to a separate service. It checks the token's account ID against the App Server account before saving data. Analytics routes are internal app endpoints and may change. Profile usage uses the experimental API capability and requires a Codex version that supports `account/usage/read`.

## Restoring a backup

Each ZIP can be opened in Finder. It contains `preferences.plist`, `Analytics/`, `Forecasts/`, and `manifest.json`. The app verifies the files and their SHA-256 hashes when it creates the ZIP. To restore a chosen snapshot:

1. Quit Codex Runway, including its login LaunchAgent if installed, and extract the ZIP.
2. Save a copy of the current `~/Library/Application Support/Codex Runway` folder. Replace its `Analytics` and `Forecasts` folders with the extracted folders.
3. Import the extracted preferences with `defaults import com.casperkristiansson.codex-runway /path/to/extracted/preferences.plist`.
4. Start Codex Runway and choose the backup folder again in Settings. The backup location and schedule are intentionally not part of the restored preferences.
