...
- Switches between the graph and a content-sized interval table. The selected range controls adaptive table intervals from 10 minutes through one day, capped at seven history rows and aligned to clean clock boundaries; each row keeps consumption visible across resets, marks resets separately, and Overview separates recent history from upcoming account resets.
